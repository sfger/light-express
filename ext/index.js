...
var reg = {
  static_dir: new RegExp(
    "^" +
      webpackConfig.context.replace( /[()^$*?+.-\\]/g, function( i ) {
        return "\\" + i;
      } )
  )
};
    ...
    try {
      list.forEach( one => {
        ...
        if ( ~[
          ...
          "html",
          "htm"
        ].indexOf( path.extname( one ).slice( 1 ) ) ) s = minify_html( s );
