$.fn.layout = function( options ) {
  ...
  handler.prototype = {
    init: function( box, options ) {
      ...
      this.panelBars = {
        north: $( ".bar-north", box )
          .css( { height: bars.north.height || size } )
          .get( 0 ),
        south: $( ".bar-south", box )
          .css( { height: bars.south.height || size } )
          .get( 0 ),
        west: $( ">.layout-middle-container>.bar-west", box )
          .css( { width: bars.west.width || size } )
          .get( 0 ),
        east: $( ">.layout-middle-container>.bar-east", box )
          .css( { width: bars.east.width || size } )
          .get( 0 )
      };
      var that = this;
      $( [
    ...
    panelResize: function() {
      ...
      $( ">.resize-bar,>.layout-middle-container>.resize-bar", box ).on( {
        mousedown: function( e ) {
          ...
          var bar = this,
            $bar = $( bar );
          if ( ( $bar.hasClass( "bar-north" ) && pops.north.resize === false ) || ( $bar.hasClass( "bar-south" ) && pops.south.resize === false ) || ( $bar.hasClass( "bar-west" ) && pops.west.resize === false ) || ( $bar.hasClass( "bar-east" ) && pops.east.resize === false ) ) return false;
    ...
    panelToggle: function( $bar ) {
      ...
      if ( ( $bar.hasClass( "bar-north" ) && pops.north.toggle === false ) || ( $bar.hasClass( "bar-south" ) && pops.south.toggle === false ) || ( $bar.hasClass( "bar-west" ) && pops.west.toggle === false ) || ( $bar.hasClass( "bar-east" ) && pops.east.toggle === false ) ) return false;
    ...
    resize: function() {
      var getElementHeight = this.getElementHeight,
        ...
        md_container_height = this.getElementHeight( this.box ) - getElementHeight( panels.north ) - getElementHeight( panels.south ) - getElementHeight( panelBars.north ) - getElementHeight( panelBars.south );
      ...
      if ( doc_mode === 5 || ie6.test( navigator.userAgent ) ) {
        var getElementWidth = this.getElementWidth,
          center_width = this.getElementWidth( this.box ) - getElementWidth( panels.west ) - getElementWidth( panels.east ) - getElementWidth( panelBars.west ) - getElementWidth( panelBars.east );
