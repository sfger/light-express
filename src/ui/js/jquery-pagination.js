$.fn.pagination = function( options ) {
  ...
  handler.prototype = {
    ...
    navishow: function( cur, page, url, show ) {
      ...
      if ( options.showPreNextOnEdge || ( options.showPreNext && cur > 1 ) ) str += this.getNaviNode( url, cur - 1, options.prePageAlias || "上一页" );
      if ( page <= show ) {
        for ( i = 1; i <= page; i++ ) {
      ...
      if ( options.showPreNextOnEdge || ( options.showPreNext && cur != page ) ) str += this.getNaviNode( url, cur + 1, options.nextPageAlias || "下一页" );
      return str + "</div>";
    }
  };
