$.fn.tree = function (options) {
  ...
  var createTree = function (data, deep, container, deepest_ul, internal_init) {
    ...
    if (!internal_init) {
      container.innerHTML =
        '<ul data-deep="0"><li class="line" style="display:block;"><a style="display:none;"><span class="title">__ROOT__</span></a></li></ul>';
    ...
    for (var i = 0, il = data.length - 1; i <= il; i++) {
      ...
      if (options.checkbox) {
        ...
        checkbox.className =
          "checkbox" + (data[i].checked ? " checkbox-all" : "");
  ...
  handler.prototype = {
    init: function (box, options) {
      ...
      if (options.checkbox) {
        ...
        // }}}
        $(this.contents).on(".checkbox", {
          click: function () {
            var line = this.parentNode;
            var li = line.parentNode;
            check.updateChildCheckState(
              { children: [li] },
              !line.option.checked
            );
      ...
      if (options.dnd) {
        var drag = {
          ...
          move: function (e) {
            ...
            if (that.draggingProxyElement.style.display !== "block")
              that.draggingProxyElement.style.display = "block";
            ...
            if (drag.prevLine)
              $(drag.prevLine).css({ border: "", boxSizing: "" });
            if (
              line &&
              line != that.draggingElement &&
              !$.contains(that.draggingElement.parentNode, line)
            ) {
          ...
          updateChildrenIndent: function (ul, gap) {
            ...
            if (!ul) return;
            var real_ul;
            if (!ul.nodeName || ul.nodeType)
              real_ul = ul.children[0].parentNode;
            else real_ul = ul;
            real_ul.setAttribute(
              "data-deep",
              Number(real_ul.parentNode.parentNode.getAttribute("data-deep")) +
                1
            );
            var lis = ul.children,
            ...
            if (drag.dropPosition) {
              if (options.onBeforeDrop) {
                var tag = options.onBeforeDrop.bind(that)(
                  drag.prevLine,
                  that.draggingElement,
                  drag.dropPosition
                );
                if (tag === false) return;
              }
              ...
              var gap =
                tli.parentNode.getAttribute("data-deep") -
                sli.parentNode.getAttribute("data-deep");
              ...
              if (drag.dropPosition === "append") {
                ...
                if (tli.parentNode === sli.parentNode) {
                  ...
                  that
                    .getParentNode(tli)
                    .option.children.splice(to_index, 0, sOption);
                } else {
    ...
    toggle: function (folder) {
      this[$(folder.nextSibling).is(":visible") ? "collapse" : "expand"](
        folder
      );
      return this;
    },
