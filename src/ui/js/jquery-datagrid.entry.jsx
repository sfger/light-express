...
function get_head_rows( _this, options, colsType ) {
  ...
  return rows.map( function( row, i ) {
    let index = 0;
    let hasRowNum = rowNum && !i && ( colsType == "frozenColumns" || ( !frozenColumns.length && colsType == "columns" ) );
    return (
      <tr key={ i }>
        { hasRowNum ? (
          <td rowSpan={ frozenColumns.length || columns.length } className="field">
        ...
        { row.map( function( option ) {
          ...
          let isField = colspan == 1 && ( i == il - 1 || il == i + option.rowspan );
...
function get_data_rows( options, cols = [], colsType ) {
  ...
  return data.map( function( row, i ) {
    let hasRowNum = rowNum && ( colsType == "frozenColumns" || ( !frozenColumns.length && colsType == "columns" ) );
    return (
      <tr key={ i }>
        {  hasRowNum ? (
          <td>
        ...
        { cols.map( ( option, i ) => {
          ...
          let width = options.autoColWidth ? "auto;" : ( option.width || options.colWidth ) + "px;";
          return (
            <td key={ i }>
              <div className="cell-wrapper">
      ...
      <div className={ "view-wrapper grid" + ( autoRowHeight ? " autoRowHeight" : "" ) }>
          ...
          <div className="col col-view frozen-view frozen-start">
            ...
            <div className="body-wrapper">
              <table className="frozen body">
                <tbody>{ get_data_rows( options, _this.frozenColumns, "frozenColumns" ) }</tbody>
              </table>
            </div>
          </div>
        ) : (
          ...
          <div>
            ...
            <div className="body-wrapper">
              <table className="body">
                <tbody>{ get_data_rows( options, _this.columns, "columns" ) }</tbody>
              </table>
            </div>
          </div>
        </div>
        { frozenEndColumns.length ? (
          <div className="col col-view frozen-view frozen-end">
            <div className="head-wrapper">
              <table className="frozen head">
                <tbody>{ get_head_rows( _this, options, "frozenEndColumns" ) }</tbody>
              </table>
            </div>
            <div className="body-wrapper">
              <table className="frozen body">
                <tbody>{ get_data_rows( options, _this.frozenEndColumns, "frozenEndColumns" ) }</tbody>
              </table>
            </div>
          </div>
        ) : (
      ...
      $box.on(
        {
          mouseenter: function( e ) {
            ...
            box._triggering_in = requestAnimationFrame( function() {
              ...
              _this.relatedData = data = _this.userOptions.data[ target.rowIndex ];
              $(
                [
                  data.frozenTr,
                  data.frozenEndTr,
