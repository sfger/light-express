(function (window) {
  ...
  var pagination = function (options) {
    ...
    var pagination = (function () {
      var pagination = function (options) {
        return new pagination.prototype.init(options);
      };
      pagination.prototype = {
        ...
        init: function (options) {
          ...
          if (!/(^|\s)pagination-ctn(\s|$)/.test(render.className)) {
            render.className +=
              (render.className ? " " : "") + "pagination-ctn";
          }
          var pageNumber, url;
          ...
          if (options.useAjax) {
            ...
            this.pageNumberRegExp = new RegExp(
              "([?&]{1}" + options.pageNumberQueryName + "=)([^&]*)"
            );
            pageNumber = this.pageNumberRegExp.exec(location.href);
            if (pageNumber && pageNumber[2]) pageNumber = pageNumber[2];
            if (!pageNumber) {
              if (location.search)
                url = location.href + "&" + options.pageNumberQueryName + "=";
              else
                url = location.href + "?" + options.pageNumberQueryName + "=";
              pageNumber = 1;
            } else {
          ...
          pageNumber =
            pageNumber > this.pageCount ? this.pageCount : pageNumber;
          options.pageNumber = pageNumber;
          this.navishow(pageNumber, this.pageCount, url, 5);
          render.innerHTML +=
            '&nbsp;<div class="form"><span>第</span><input name="' +
            options.pageNumberQueryName +
            '" class="page" type="text" value="" /><a href="javascript:;" class="go">GO</a><span>页</span></div>&nbsp;<div class="desc">共<span class="dataSize">' +
            options.dataSize +
            "</span>条数据</div>";
          this.initEvent();
        },
        initEvent: function () {
          ...
          render.children[1].children[0].onsubmit = function () {
            var a = window.getElementsByClassName("page", render)[0].value;
            if (a > that.pageCount)
              window.getElementsByClassName("page", render)[0].value =
                that.pageCount;
            else if (a < 1)
              window.getElementsByClassName("page", render)[0].value = 1;
            if (isNaN(a)) a = 1;
            if (options.useAjax) {
              options.pageNumber = Number(a);
              if (options.onChangePage)
                options.onChangePage.call(
                  that,
                  options.pageNumber,
                  that.pageCount
                );
        ...
        appendPlainChild: function (text) {
          this.userOptions.render.children[0].innerHTML +=
        ...
        navishow: function (cur, page, url, show) {
          ...
          if (page <= show) {
            ...
            if (cur - 2 < hf + 2) {
              ...
              for (
                i = cur - hf + (page - cur - hf > 0 ? 0 : page - cur - hf);
                i <= cur;
                i++
              )
                if (i == cur) this.appendPlainChild(i);
    ...
    for (var i = 0; i < options.renders.length; i++) {
      ...
      options.renders[i].ui = {
        iPagination: pagination(
          $.extend(true, { render: options.renders[i] }, options)
        ),
      };
    }
  };
