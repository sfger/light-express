	...
	handler.prototype = {
					...
					var document_events = {
						...
						'mouseup': function(e){
							...
							if($bar.hasClass('bar-west')){
								...
								resize_box.css({width:that.getViewWidth()-x-(barOps.east.width||barSize)});
							}else if($bar.hasClass('bar-north')){
								resize_box.css({height:y});
							}else if($bar.hasClass('bar-south')){
								resize_box.css({height:that.getViewHeight()-y-(barOps.south.height||barSize)});
							}
		...
		resize: function(){
			var getElementHeight    = this.getElementHeight,
				...
				md_container_height = this.getViewHeight()
