...
require("../../public/js/requestAnimationFrame");
var fn_1 = require("../../public/js/parts/fn");
$.fn.datagrid = function (options) {
    ...
    if ('string' === $.type(options)) {
        ...
        var ret = this.toArray().map(function (one) {
            var ui = $(one).data('ui');
            if (ui && ui.iDatagrid) {
    ...
    function get_table(options, that) {
        function get_head_rows(rows, colsType) {
            if (!rows || (colsType == 'frozenColumns') && !options.frozenColumns.length)
                return [];
            if (!rows || (colsType == 'frozenEndColumns') && !options.frozenEndColumns.length)
                return [];
            var il = rows.length - 1;
            return rows.map(function (row, i) {
                return fn_1.createElement({ name: 'tr', children: (function () {
                        var index = 0;
                        var nodes = row.map(function (option /*, j*/) {
                            var title = (option.name || option.field || '');
                            var td_attr = {};
                            if (option.rowspan)
                                td_attr['rowspan'] = option.rowspan;
                            if (option.colspan)
                                td_attr['colspan'] = option.colspan;
                        ...
                        if (options.rowNum && !i)
                            if (options.frozenColumns.length && (colsType == 'frozenColumns')
                                ...
                                nodes.unshift(fn_1.createElement({
        ...
        function get_data_rows(data, cols, colsType) {
            if ((colsType === 'frozenColumns') && !options.frozenColumns.length)
                return [];
                ...
                return fn_1.createElement({
                    name: 'tr', children: (function () {
                        ...
                        cols && cols.forEach(function (option) {
                            if (!option)
                                return true;
                            var field = option.field, val = row[field], formatter = option.formatter, cls_list = ['cell'], align = ({ 'left': 'txt-lt', 'right': 'txt-rt' })[option.align] || '';
    ...
    function resize_table(that) {
        ...
        function update_scroll_offset() {
            var aview = tp0.get(0).parentNode;
            aview.scrollLeft = this.scrollLeft;
            if (options.frozenColumns.length) {
                var fview = tables.get(1).parentNode;
                fview.scrollTop = this.scrollTop;
            }
            if (options.frozenEndColumns.length) {
                var feview = $('.frozen-end table', that.render).get(1).parentNode;
        ...
        if (options.frozenColumns || options.frozenEndColumns) {
            $([tables[1], tables[5]]).off('mousewheel DOMMouseScroll').on('mousewheel DOMMouseScroll', function (e) {
                ...
                if (tables.get(5))
                    list.push(tables.get(5).parentNode);
                if ($(list).is(':animated'))
                    return false;
        ...
        requestAnimationFrame(function () {
            var item = $autoView.find('.body-wrapper')[0];
            var bar_width = item.offsetWidth - item.clientWidth;
    ...
    handler.prototype = {
        ...
        update: function (options) {
                ...
                if (options.data) {
                    ...
                    if (data[0].tr || data[0].frozenTr || data[0].frozenEndTr) {
                        data.forEach(function (rowData) {
                            delete rowData.tr;
                            if (old_options.frozenColumns.length)
                                delete rowData.frozenTr;
                            if (old_options.frozenEndColumns.length)
                                delete rowData.frozenEndTr;
                        });
                    }
            ...
            options.data.forEach(function (rowData, rowNum) {
                if (options.frozenColumns.length) {
                    var tbody = $('.frozen-view .body tbody', box)[0];
                    rowData.frozenTr = tbody.rows[rowNum];
                }
                if (options.frozenEndColumns.length) {
                    var tbody = $('.frozen-end .body tbody', box)[0];
                    rowData.frozenEndTr = tbody.rows[rowNum];
                }
                if (options.columns.length) {
                    var tbody = $('.auto-view .body tbody', box)[0];
        ...
        init_event: function (options) {
            ...
            if (options.triggerRow && (options.frozenColumns.length || options.frozenEndColumns.length))
                $box.on({
                    mouseenter: function (e) {
                        ...
                        box._triggering_in = requestAnimationFrame(function () {
                            var data = that.relatedData;
                            if (data)
                                $([data.frozenTr, data.frozenEndTr, data.tr].filter(function (item) { return item; })).removeClass('hover');
        ...
        sortBy: function (option, sortElement) {
            ...
            if (preSortElement) {
                if (sortElement === preSortElement) {
                    if (option.order === preSortElement.order)
                        return this; // 排序没变中断
        ...
        sort_table_dom: function (options) {
            var frozenTrDoc = document.createDocumentFragment(), frozenEndTrDoc = document.createDocumentFragment(), trDoc = document.createDocumentFragment(), frozenTbody = null, frozenEndTbody = null, tbody = null;
            options.data.forEach(function (rowData, rowNum) {
                ...
                if (frozenTr) {
                    ...
                    if (options.rowNum)
                        $('td:eq(0) .cell', frozenTr).text(rowNum + 1);
                }
                if (frozenEndTr) {
    ...
    return this.each(function () {
        ...
        var ui = $this.data('ui');
        if (ui)
            ui.iDatagrid = instance;
        else
            $this.data('ui', { iDatagrid: instance });
    }); //}}}
};
// vim: fdm=marker
